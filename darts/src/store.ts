...
export function getPlayerWithStats(playerId: string): PlayerWithStats | null {
  ...
  const avg01 = stats01Values.length > 0
    ...
    : 0;
  // Normalize 01 avg (typical DartsLive range ~10-20) to 0-100
  const normalized01 = Math.min(100, Math.max(0, (avg01 - 10) * 10));
...
export function generateFullLineup(
  ...
  // Part 3 (G8-G9): 1 repeat from Parts 1-2 allowed
  const parts = [
  ...
  for (let p = 0; p < parts.length; p++) {
    ...
    if (p === 0) {
      ...
      // Parts 2-3: remove players used in previous parts, then add back top repeat candidates
      const prevUsed = new Set(partUsedPlayers.flatMap(s => Array.from(s)));
      partPool = partPool.filter(pp => !prevUsed.has(pp.player.id));

      if (repeatSlots > 0) {
        // Find top candidates from previous part by composite score
...
function formatScore(player: PlayerWithStats, legs: string): number {
  ...
  if (isOnly01) {
    // Pure 01 game (G1, G9) — use normalized 01 average
    return player.stats01Avg > 0 ? Math.min(100, Math.max(0, (player.stats01Avg - 10) * 10)) : 0;
  }
  if (isOnlyCricket) {
...
export function getPlayerDashboardStats() {
  return getPlayers().map(p => {
    ...
    const games = gps.length;
    const wins = gps.filter(g => g.won).length;
    const losses = games - wins;
    const stats01Values = gps.filter(g => g.stats01 !== undefined).map(g => g.stats01!);
    const statsCricketValues = gps.filter(g => g.statsCricket !== undefined).map(g => g.statsCricket!);
    const stats01Avg = stats01Values.length > 0 ? stats01Values.reduce((a, b) => a + b, 0) / stats01Values.length : 0;
    const statsCricketAvg = statsCricketValues.length > 0 ? statsCricketValues.reduce((a, b) => a + b, 0) / statsCricketValues.length : 0;
    return {
      player: p,
      games, wins, losses,
...
export function populateFromLiveData(liveData: LiveDataInput) {
  ...
  const players = liveData.players.map((p) => {
    return {
      ...
      name: p.name,
      rating: 0,
      notes: `01: ${p.stats01Avg.toFixed(2)} · Cricket: ${p.statsCricketAvg.toFixed(2)}`,
  ...
  const sessions = liveData.matches.map((m, i) => {
    ...
    return {
      ...
      date: m.matchDate,
      type: 'match' as const,
      notes: `W${i + 1} ${m.isThirstdayHome ? 'vs' : '@'} ${opponent} (${result} ${thirstdayScore}-${opponentScore})`,
...
export function updateFromLiveData(liveData: LiveDataInput): number {
  ...
  for (const ap of liveData.players) {
    if (playerMap.has(ap.name)) {
      // Update existing player's notes with latest stats
      const existing = playerMap.get(ap.name)!;
      existing.notes = `01: ${ap.stats01Avg.toFixed(2)} · Cricket: ${ap.statsCricketAvg.toFixed(2)}`;
      ...
      const newPlayer = {
        ...
        name: ap.name,
        rating: 0,
        notes: `01: ${ap.stats01Avg.toFixed(2)} · Cricket: ${ap.statsCricketAvg.toFixed(2)}`,
        createdAt: new Date().toISOString(),
      };
